...
async def create_user_property(
    ...
    try:
        ...
        properties = get_user_properties(user_id, couchdb, configuration)
        ...
        save_user_property(user_id, storage_key, property_data, couchdb, configuration)
...
async def update_user_property(
    ...
    try:
        ...
        save_user_property(user_id, property_name, property_data, couchdb, configuration)
...
async def delete_user_property(
    ...
    try:
        ...
        users_db = couchdb[configuration.get_string('Databases', 'CouchDB', 'Databases', 'Users')]
        user_doc = users_db.get(user_id)

        if not user_doc or property_name not in user_doc['additional_properties']:
            raise HTTPException(
                status_code=status.HTTP_404_NOT_FOUND,
                detail=f"Property '{property_name}' not found"
            )
